...
function Hero() {
  ...
  useEffect(() => {
    ...
    if (images.length > 0) {
      ...
      setHeroImage(transformCloudinaryUrl(images[randomIndex], 576, 576));
    }
  ...
  const techIcons = [
    ...
    { icon: "logos:bootstrap", position: "bottom-[-1.5rem] left-1/2 -translate-x-1/2", },
  ...
  return (
    <div className="hero min-h-screen bg-base-100 flex items-center justify-center pt-16" id="home">
      <SeoHelmet title="Mazda Nawallsyah" description="Portofolio pribadi Mazda Nawallsyah seorang Fresh Graduate S1 - Teknik Informatika Universitas Semarang yang berfokus di Bidang Front-End Web Dev." imageUrl={heroImage} url="/" />

      <div className="hero-content flex flex-col lg:flex-row-reverse items-center justify-between w-full max-w-6xl mx-auto px-4">

        <div className="relative mb-8 lg:mb-0 lg:ml-10 animate-float group will-change-transform" data-aos="fade-left" data-aos-delay="300">
          <div tabIndex={0} className="avatar">
            <div className="w-56 h-56 md:w-72 md:h-72 aspect-square rounded-full ring-7 ring-base-300 ring-offset-base-100 ring-offset-4 overflow-hidden relative bg-base-300">
              ...
              <img
                ...
                width="288"
                height="288"
                className={`w-full h-full object-cover transition-all duration-300 ease-in-out cursor-pointer ${imageLoaded ? "opacity-100 group-hover:scale-115" : "opacity-0"
                  }`}
          ...
          {techIcons.map((tech, index) => (
            <div key={index} data-aos="zoom-in" data-aos-delay={300 + index * 100} className={`absolute ${tech.position} bg-base-300 p-2 rounded-full shadow-lg`}>
              <Icon icon={tech.icon} className="w-6 h-6 md:w-8 md:h-8 drop-shadow-lg" />
            </div>
          ))}
        </div>

        <div className="flex flex-row items-start max-w-lg text-center lg:text-left" data-aos="fade-right" data-aos-duration="700">

          <div className="hidden sm:flex flex-col space-y-4 mr-6 mt-2 min-w-[24px]">
            {imageLoaded ? (
              <div data-aos="fade-right" data-aos-delay="500" className="flex flex-col space-y-4">
              ...
              <div className="flex flex-col space-y-4">
                {[1, 2, 3].map(i => <div key={i} className="skeleton h-6 w-6 rounded-full"></div>)}
              </div>
            )}
          </div>

          <div className="transition-opacity duration-500 w-full" data-aos="zoom-out">
            {!imageLoaded ? (
              <div className="space-y-4">
                <div className="skeleton h-8 md:h-10 w-3/4 mx-auto lg:mx-0"></div>
                <div className="skeleton h-7 w-1/2 mx-auto lg:mx-0"></div>
                <div className="py-6 space-y-2">
                  <div className="skeleton h-4 w-full"></div><div className="skeleton h-4 w-full"></div><div className="skeleton h-4 w-5/6"></div>
                </div>
              ...
              <>
                ...
                <p className="py-6 text-base md:text-lg text-base-content/80 text-justify min-h-[80px]">
                  {displayParagraph}
                </p>
                ...
                <div tabIndex={0} className="flex sm:hidden space-x-4 mt-6 justify-center lg:justify-start min-h-[24px]">
                  {socialLinkConfig.filter((link) => availableLinks[link.key]).map((link) => (
                    <a key={link.key} href={link.baseUrl + availableLinks[link.key]} target="_blank" rel="noopener noreferrer" aria-label={link.label} className="text-base-content/70 hover:text-primary transition-colors duration-200 group-focus-within:text-secondary">
                      <Icon icon={link.icon} className="w-6 h-6" />
                    </a>
                  ))}
                </div>
