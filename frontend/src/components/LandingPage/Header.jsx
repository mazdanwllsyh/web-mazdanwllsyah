        ...
        <div className="flex items-center justify-between">
          <div className="flex-1">
            <a href="/" className="flex items-center gap-2 group w-fit">
              <div
                className={`rounded-2xl bg-gradient-to-br from-accent to-primary text-base-100 flex items-center justify-center font-black transition-all duration-500 ${isScrolled ? "w-8 h-8 text-sm" : "w-10 h-10 text-lg"}`}
              >
                {siteData?.brandName?.charAt(0) || "M"}
              </div>
              <span
                className={`font-display font-black tracking-tighter transition-all duration-500 ${isScrolled ? "text-base" : "text-xl"}`}
              >
          ...
          <div className="flex-1 flex items-center justify-end gap-2 md:gap-4">
            ...
            {user ? (
              <div className="dropdown dropdown-end">
                ...
                <ul
                  tabIndex={0}
                  className="mt-4 z-[1] p-2 shadow-2xl menu menu-md dropdown-content bg-base-100 rounded-2xl w-64 border border-base-content/10 animate-fade-in-up"
                >
                  <li className="menu-title px-4 py-3 border-b border-base-content/5 mb-2 text-xs opacity-50 uppercase font-bold text-base-content">
                    Akun Saya
                  </li>
                  <li>
                    <Link
            ...
            <div className="dropdown dropdown-end xl:hidden">
              ...
              <ul
                tabIndex={0}
                className="menu menu-md dropdown-content mt-4 z-[1] p-3 shadow-2xl bg-base-100 rounded-2xl w-60 border border-base-content/10"
              >
                {navLinks.map((link) => {
                  const isAtHome = location.pathname === "/";
                  const isActive =
