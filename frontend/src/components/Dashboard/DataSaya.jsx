...
function DataSaya() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showSuccessSwal(
        ...
        "Informasi dasar dan tautan kontak telah diperbarui.",
      );
    } catch (error) {
  ...
  const handleDeleteImage = async (index, imageUrl) => {
    const isConfirmed = await showConfirmSwal(
      "Hapus Gambar?",
      "Gambar profil ini akan dihapus secara permanen.",
    );
    if (isConfirmed) {
      setIsDeleting(index);
      try {
      ...
      >
        <div className="lg:col-span-8 grid grid-cols-1 gap-6">
                ...
                <div className="space-y-3">
                  <div className="flex gap-2">
                    <FloatingLabelInput
                      ...
                      onKeyPress={(e) =>
                        e.key === "Enter" && (e.preventDefault(), addTypeSeq())
                      }
                    />
                    <button
                      type="button"
                      onClick={addTypeSeq}
                      className="btn btn-primary h-14 w-14 rounded-xl"
                    >
                      <Icon icon="solar:add-circle-bold" className="w-6 h-6" />
                    </button>
                  </div>
                  <div className="flex flex-wrap gap-2 p-3 border border-primary/30 rounded-xl bg-base-200/30 min-h-14 items-center">
                    {typeSequences.length > 0 ? (
                      typeSequences.map((text, i) => (
                        <div
                          key={i}
                          className="badge badge-lg py-4 px-4 bg-base-100 border-primary/30 gap-2 font-bold text-xs tracking-wider animate-in fade-in zoom-in duration-300"
                        >
                          {text}
                          <Icon
                            icon="solar:close-circle-bold"
                            className="w-4 h-4 cursor-pointer text-error hover:scale-110 transition-transform"
                            onClick={() => removeTypeSeq(i)}
                          />
                        </div>
                      ))
                    ) : (
                      <span className="text-xs italic opacity-40 ml-2">
                        Belum ada sequence, ketik lalu tekan Enter atau klik
                        ikon +
                      </span>
                    )}
                  </div>
          ...
          <div className="flex justify-center pt-2">
            <button
              type="submit"
              className="btn btn-primary btn-lg w-full md:w-80 rounded-2xl shadow-lg shadow-primary/20"
              disabled={isSaving}
            >
              {isSaving ? (
                <span className="loading loading-ring loading-md"></span>
              ) : (
                <Icon icon="solar:diskette-bold-duotone" className="w-6 h-6" />
              )}
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                  {formData.profileImages.map((imageUrl, index) => (
                    <div
                      key={index}
                      className="group relative rounded-2xl border border-base-300 overflow-hidden bg-base-200/50 p-2 flex items-center gap-4 hover:border-error/30 transition-all"
                    >
                      ...
                      <button
                        ...
                        className="btn btn-square btn-error btn-sm opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity"
                ...
                <div className="p-4 bg-info/10 text-info rounded-xl flex items-start gap-3">
                  ...
                  <p className="text-[10px] leading-relaxed font-medium">
                    Gambar ini akan muncul secara bergantian di bagian{" "}
                    <strong>About</strong> dan <strong>Hero Section</strong>{" "}
                    sebagai slideshow dinamis.
                  </p>
                </div>
              </div>
            </div>
